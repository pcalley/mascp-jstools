...
return result_data;};var splice_char=function(seqs,index,insertions){for(var i=0;i<seqs.length;i++){var seq=seqs[i];if(seq.charAt(index)!='-'){if(!insertions[i]){insertions[i]={};}
...
MASCP.ClustalRunner.Result.prototype.alignToSequence=function(seq_index){var seqs=this._raw_data.data.sequences.concat([this._raw_data.data.alignment]);var insertions=[];var aligning_seq=seqs[seq_index],i=aligning_seq.length;for(i;i>=0;i--){if(aligning_seq.charAt(i)=='-'){splice_char(seqs,i,insertions);}}
