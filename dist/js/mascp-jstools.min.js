...
{var reader=this;this.bind('resultReceived',function(){sequenceRenderer._peptide_sequences['ppdb']=[];var thesePeptides=this.result.getPeptides();for(var k=0;k<thesePeptides.length;k++){sequenceRenderer._peptide_sequences['ppdb'].push(thesePeptides[k].sequence);}
MASCP.registerGroup('ppdb',{'fullname':'PPDB spectra data','hide_member_controllers':true,'hide_group_controller':true,'color':'#aa9900'});var overlay_name='ppdb_controller';var css_block='.active .overlay { background: #aa9900; } .active a { color: #000000; text-decoration: none !important; }  :indeterminate { background: #ff0000; } .tracks .active { background: #0000ff; } .inactive a { text-decoration: none; } .inactive { display: none; }';MASCP.registerLayer(overlay_name,{'fullname':'PPDB MS/MS','color':'#aa9900','css':css_block});if(sequenceRenderer.createGroupController){sequenceRenderer.createGroupController('ppdb_controller','ppdb');}
...
{var reader=this;this.bind('resultReceived',function(){sequenceRenderer._peptide_sequences['proteotypic']=[];var thesePeptides=this.result.getPeptides();for(var k=0;k<thesePeptides.length;k++){sequenceRenderer._peptide_sequences['proteotypic'].push(thesePeptides[k].sequence);}
MASCP.registerGroup('proteotypic_experimental',{'fullname':'Expected Peptides','hide_member_controllers':true,'hide_group_controller':true,'color':'#ff5533'});var overlay_name='proteotypic_controller';var css_block='.active .overlay { background: #ff5533; } .active a { color: #000000; text-decoration: none !important; }  :indeterminate { background: #ff0000; } .tracks .active { background: #0000ff; } .inactive a { text-decoration: none; } .inactive { display: none; }';MASCP.registerLayer(overlay_name,{'fullname':'Predicted Peptides','color':'#008000','css':css_block});if(sequenceRenderer.createGroupController){sequenceRenderer.createGroupController('proteotypic_controller','proteotypic_experimental');}
var peps=this.result.getPeptides();var exps_done='';for(var i=0;i<peps.length;i++){var an_exp=peps[i].exp;if(exps_done.search(an_exp)<0){MASCP.registerLayer('proteotypic_peptide_'+an_exp,{'fullname':an_exp.replace('_','/'),'group':'proteotypic_experimental','color':'#008000','css':css_block});exps_done=exps_done+an_exp;}
...
return gradient;};canvas.mod_gradient=function(id,locations,predictedLocations,maxRating){var gradient=this.makeEl('linearGradient',{'id':id,'x1':'0%','x2':'100%','y1':'0%','y2':'0%'});while(locations.length>0){var locate=locations.shift();var isPredicted=false;for(var i=0;i<predictedLocations.length;i++){var a=locate[0];var b=locate[1];var c=predictedLocations[i][0];var d=predictedLocations[i][1];if((c<=a&&d>=a&&d-a>=(b-a)*0.5)||(c<=b&&c>=a&&b-c>=(b-a)*0.5)){isPredicted=true;}}
var thisRating=9-(maxRating+(isPredicted==true?5:0));var colorStr=''+thisRating+thisRating;console.log(colorStr);if(locate[0]>=0.005){gradient.appendChild(this.makeEl('stop',{'offset':(locate[0]-0.005),'style':'stop-color:#ccc;stop-opacity:1'}));}
gradient.appendChild(this.makeEl('stop',{'offset':(locate[0]>0)?(locate[0]+0.005):0,'style':'stop-color:#f'+colorStr+';stop-opacity:1'}));gradient.appendChild(this.makeEl('stop',{'offset':(locate[1]<1)?(locate[1]-0.005):1,'style':'stop-color:#f'+colorStr+';stop-opacity:1'}));if(locate[1]<=0.995){gradient.appendChild(this.makeEl('stop',{'offset':(locate[1]+0.005),'style':'stop-color:#ccc;stop-opacity:1'}));}}
...
return ret;};var invertIndeces=function(inputIndex){var results=[];var inputLength=inputIndex.length;for(var n=0;n<inputLength;n++){if(n==0&&inputIndex[n][0]>0){results.push([0,Math.max(inputIndex[n][0]-1,0)]);}
...
return results;};var toPercent=function(indecesList){var results=[];var convertIndeces=function(inputIndex){var left=(inputIndex[0]/seqLength);var right=(inputIndex[1]/seqLength);return[Math.max(left-0.005,0),right-0.005];};if(indecesList[0][0]>5){results.push(convertIndeces([0,indecesList[0][0]]));}
for(var l=0;l<(indecesList.length-1);l++){if(indecesList[l+1][0]-indecesList[l][1]>5){results.push(convertIndeces([indecesList[l][1],indecesList[l+1][0]]));}}
if(seqLength-indecesList[indecesList.length-1][1]>5){results.push(convertIndeces([indecesList[indecesList.length-1][1],seqLength]));}
return results;};var consolidatePeptides=function(arr){var coveredIndeces=[];var results=[];for(var i=0;i<arr.length;i++){var hitIndex=sequence.indexOf(arr[i]);if(hitIndex>=0){coveredIndeces.push([hitIndex,(hitIndex+arr[i].length)]);}}
for(var j=0;j<(coveredIndeces.length-1);j++){var toDelete=[];for(var k=j+1;k<coveredIndeces.length;k++){var a=coveredIndeces[j][0];var b=coveredIndeces[j][1];var c=coveredIndeces[k][0];var d=coveredIndeces[k][1];var hasChanged=false;if((coveredIndeces[j]==coveredIndeces[k])||(a<=c&&b>=d)){coveredIndeces.splice(k,1);k--;}
else if((a<=c&&(b<=d&&b>=c))||((a>=c&&a<=d)&&b>=d)||(a>=c&&b<=d)){coveredIndeces[j][0]=Math.min(a,c);coveredIndeces[j][1]=Math.max(b,d);coveredIndeces.splice(k,1);hasChanged=true;}
if(hasChanged==true){j=-1;break;}}}
coveredIndeces.sort(function(a,b){if(a[0]<b[0]){return-1;}else{return 1;}});return coveredIndeces;};var peps=[];var predictedPeps=[];for(var reader in this._peptide_sequences){for(var l=0;l<this._peptide_sequences[reader].length;l++){if(reader=='proteotypic'){predictedPeps.push(this._peptide_sequences[reader][l]);}else{peps.push(this._peptide_sequences[reader][l]);}}}
if(peps.length>0){var maxRating=peps.length/40;maxRating=(maxRating>1)?1:maxRating;maxRating=Math.round(maxRating*4);peps=sort_unique(peps);peps=consolidatePeptides(peps);var percentages=toPercent(peps);var predictedPercent=[];if(predictedPeps.length>0){predictedPeps=sort_unique(predictedPeps);predictedPeps=consolidatePeptides(predictedPeps);predictedPeps=invertIndeces(predictedPeps);predictedPercent=toPercent(predictedPeps);}
var canv=this._canvas;var defs=canv.parentNode.ownerDocument.getElementsByTagNameNS(svgns,'defs')[0];defs.appendChild(canv.mod_gradient('mod_gradient',percentages,predictedPercent,maxRating));}
else{var modLayer=MASCP.getLayer('modhunter');this.removeTrack(modLayer);}
