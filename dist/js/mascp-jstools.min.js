...
{var reader=this;this.bind('resultReceived',function(){for(var k=0;k<this.result.getPeptides().length;k++){sequenceRenderer._peptide_sequences.push(this.result.getPeptides()[k].sequence);}
...
return ret;};this.bind('resultReceived',function(){for(var maps=this.result.maps,j=maps.length-1;j>=0;j--){var a_map=maps[j];MASCP.registerLayer('gelmap_map_'+a_map.id,{'fullname':a_map.title,'group':'gelmap_experimental','color':'#aaaaff','css':css_block});MASCP.getLayer('gelmap_map_'+a_map.id).href=a_map.url;var peps=sort_unique(maps[j].peptides);for(var i=peps.length-1;i>=0;i--){var peptide=peps[i];sequenceRenderer._peptide_sequences.push(peptide);var peptide_bits=sequenceRenderer.getAminoAcidsByPeptide(peptide);var layer_name='gelmap_map_'+a_map.id;peptide_bits.addToLayer(layer_name);peptide_bits.addToLayer(controller_name);}}
...
{var reader=this;this.bind('resultReceived',function(){for(var k=0;k<this.result.getPeptides().length;k++){sequenceRenderer._peptide_sequences.push(this.result.getPeptides()[k]);}
...
return gradient;};canvas.mod_gradient=function(id,locations,maxRating){var gradient=this.makeEl('linearGradient',{'id':id,'x1':'0%','x2':'100%','y1':'0%','y2':'0%'});var colorStr=''+(9-maxRating)+(9-maxRating);while(locations.length>0){var locate=locations.shift();if(locate[0]>=0.01){gradient.appendChild(this.makeEl('stop',{'offset':(locate[0]-0.01),'style':'stop-color:#ccc;stop-opacity:1'}));}
gradient.appendChild(this.makeEl('stop',{'offset':(locate[0]>0)?(locate[0]+0.01):0,'style':'stop-color:#f'+colorStr+';stop-opacity:1'}));gradient.appendChild(this.makeEl('stop',{'offset':(locate[1]<1)?(locate[1]-0.01):1,'style':'stop-color:#f'+colorStr+';stop-opacity:1'}));if(locate[1]<=0.99){gradient.appendChild(this.makeEl('stop',{'offset':(locate[1]+0.01),'style':'stop-color:#ccc;stop-opacity:1'}));}}
...
MASCP.registerLayer('modhunter',{'fullname':'Mod Hunter','color':'#990000'});var rect=this._canvas.rect(0,0,this._sequence_els.length,2);rect.setAttribute('fill','url(#mod_gradient)');this._layer_containers.modhunter.push(rect);return this;};MASCP.CondensedSequenceRenderer.prototype.setModhunterGradient=function(){var sequence=this.sequence;var sort_unique=function(arr){arr=arr.sort();var ret=[arr[0]];for(var i=1;i<arr.length;i++){if(arr[i-1]!==arr[i]){ret.push(arr[i]);}}
return ret;};var findUncoveredRegions=function(arr){var coveredIndeces=[];var results=[];var seqLength=sequence.length;var toPercent=function(indeces){var left=(indeces[0]/seqLength);var right=(indeces[1]/seqLength);return[Math.max(left-0.005,0),right-0.005];};for(var i=0;i<arr.length;i++){var hitIndex=sequence.indexOf(arr[i]);if(hitIndex>=0){coveredIndeces.push([hitIndex,(hitIndex+arr[i].length)]);}}
...
coveredIndeces.sort(function(a,b){if(a[0]<b[0]){return-1;}else{return 1;}});if(coveredIndeces[0][0]>10){results.push(toPercent([0,coveredIndeces[0][0]]));}
for(var l=0;l<(coveredIndeces.length-1);l++){if(coveredIndeces[l+1][0]-coveredIndeces[l][1]>10){results.push(toPercent([coveredIndeces[l][1],coveredIndeces[l+1][0]]));}}
if(seqLength-coveredIndeces[coveredIndeces.length-1][1]>10){results.push(toPercent([coveredIndeces[coveredIndeces.length-1][1],seqLength]));}
return results;};var peps=this._peptide_sequences;if(peps.length>0){peps=sort_unique(peps);var maxRating=peps.length/40;maxRating=(maxRating>1)?1:maxRating;maxRating=Math.round(maxRating*9);var percentages=findUncoveredRegions(peps);var canv=this._canvas;var defs=canv.parentNode.ownerDocument.getElementsByTagNameNS(svgns,'defs')[0];defs.appendChild(canv.mod_gradient('mod_gradient',percentages,maxRating));}
