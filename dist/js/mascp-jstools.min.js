...
a_text.replaceChild(document.createTextNode(renderer.sequence.substr(start,1500)),a_text.firstChild);a_text.setAttribute('dx',5+((start)*RS));};canvas.addEventListener('panstart',function(){amino_acids.attr({'y':'-1000'});jQuery(canvas).bind('panend',function(){if(amino_acids_shown){amino_acids.attr({'y':12*RS});update_sequence();}
...
if(self.zoomCenter=='center'){self.zoomCenter={'x':0.5*self.sequence.length*self._RS*((self._canvas.width.baseVal.value+self._canvas.currentTranslate.x)/self._canvas.width.baseVal.value)};}
