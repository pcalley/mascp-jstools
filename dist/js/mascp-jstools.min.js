...
axis.show();axis.attr({'stroke-width':RS+'pt'});big_ticks.show();big_ticks.attr({'stroke-width':RS+'pt','transform':'','stroke':'#000000'});big_labels.show();big_labels.attr({'font-size':7*RS+'pt','y':5*RS});little_ticks.hide();little_labels.hide();}},false);};clazz.prototype.leftVisibleResidue=function(){var self=this;return 20+self.sequence.length*(1-((self._canvas.width.baseVal.value+self._canvas.currentTranslate.x)/self._canvas.width.baseVal.value));};clazz.prototype.rightVisibleResidue=function(){var self=this;return self.leftVisibleResidue()+self.sequence.length*(self._container_canvas.width.baseVal.value/self._canvas.width.baseVal.value);};clazz.prototype.setSequence=function(sequence){var new_sequence=this._cleanSequence(sequence);if(new_sequence==this.sequence&&new_sequence!==null){jQuery(this).trigger('sequenceChange');return;}
