...
{};if(typeof MASCP=='undefined'||typeof MASCP.Service=='undefined'){throw"MASCP.Service is not defined, required class";}
MASCP.GoogledataReader=MASCP.buildService(function(data){if(!data){return this;}
this.data=data?data.data:data;(function(self){self.getPeptides=function(){return data.data;};})(this);return this;});MASCP.GoogledataReader.prototype.toString=function(){return'MASCP.GoogledataReader.'+this.datasetname;};(function(){var attach_google_scripts=function(){var head=document.getElementsByTagName('head')[0];var script=document.createElement('script');script.type='text/javascript';script.src="http://www.google.com/jsapi";head.appendChild(script);return;};if(typeof google=='undefined'){attach_google_scripts();}
var scope="https://docs.google.com/feeds/ https://spreadsheets.google.com/feeds/";var authenticate=function(){if(!google.accounts.user.checkLogin(scope)==''){return true;}else{google.accounts.user.login(scope);}
...
var get_document=function(doc,callback){if(!doc.match(/^spreadsheet/)){console.log("No support for retrieving things that aren't spreadsheets yet");return;}
var doc_id=doc.replace(/^spreadsheet:/,'');get_document_using_script(doc_id,function(err,dat){if(err){authenticate();var feedUrl="https://spreadsheets.google.com/feeds/cells/"+doc_id+"/1/private/basic";var service=new google.gdata.client.GoogleService('wise','gator');service.getFeed(feedUrl,function(data){callback.call(null,null,parsedata(data));},callback);}else{callback.call(null,null,dat);}});};MASCP.GoogledataReader.prototype.getDocumentList=documents;MASCP.GoogledataReader.prototype.getDocument=get_document;})();MASCP.GoogledataReader.prototype.setupSequenceRenderer=function(renderer){};if(typeof MASCP=='undefined'||typeof MASCP.Service=='undefined'){throw"MASCP.Service is not defined, required class";}
...
return defaultDataReceived.call(this,data,status);};})(MASCP.ClustalRunner);(function(){var original_sequences=null;var original_alignment=null;var insertions=null;var normalise_insertions=function(inserts){var pos;var positions=[];var result_data={};for(pos in inserts){if(inserts.hasOwnProperty(pos)){positions.push(pos);}}
positions=positions.sort();for(var i=positions.length-1;i>=0;i--){var j=i-1;pos=positions[i];var value=inserts[pos];while(j>=0){pos-=inserts[positions[j]];j--;}
result_data[pos]=value;}
return result_data;};var splice_char=function(seqs,index){for(var i=0;i<seqs.length;i++){var seq=seqs[i];if(seq.charAt(index)!='-'){if(!insertions[i]){insertions[i]={};}
insertions[i][index-1]=1;if(insertions[i][index]){insertions[i][index-1]+=insertions[i][index];delete insertions[i][index];}}
seqs[i]=seq.slice(0,index)+seq.slice(index+1);}}
MASCP.ClustalRunner.Result.prototype.alignToSequence=function(seq_index){if(!seq_index&&original_sequences){this._raw_data.data.sequences=original_sequences;this._raw_data.data.alignment=original_alignment;}
if(!original_sequences){original_sequences=this._raw_data.data.sequences;original_alignment=this._raw_data.data.alignment;}
var seqs=this._raw_data.data.sequences.concat([this._raw_data.data.alignment]);insertions=[];var aligning_seq=seqs[seq_index],i=aligning_seq.length;for(i;i>=0;i--){if(aligning_seq.charAt(i)=='-'){splice_char(seqs,i);}}
...
this._raw_data.data.alignment=seqs.pop();this._raw_data.data.sequences=seqs;};})();MASCP.ClustalRunner.Result.prototype.getSequences=function(){if(this._raw_data.data.sequences){return[].concat(this._raw_data.data.sequences);}
...
return results;};MASCP.ClustalRunner.Result.prototype.getAlignment=function(){if(this._raw_data.data.alignment){return this._raw_data.data.alignment.toString();}
...
if((start+max_length)>=seq.length){start=seq.length-max_length-1;}
