...
var peps=this.result.getPeptides();sequenceRenderer._peptide_sequences['atpeptide']=[];for(var j=0;j<this.result.tissues().length;j++){var a_tissue=this.result.tissues()[j];MASCP.registerLayer('atpeptide_peptide_'+a_tissue,{'fullname':this.result._long_name_map[a_tissue],'group':'atpeptide_experimental','color':'#ff5533','css':css_block});for(var i=0;i<peps.length;i++){var peptide=peps[i].sequence;if(peps[i].tissues.indexOf(a_tissue+'')<0){continue;}
var peptide_bits=sequenceRenderer.getAminoAcidsByPeptide(peptide);var layer_name='atpeptide_peptide_'+a_tissue;peptide_bits.addToLayer(layer_name);peptide_bits.addToLayer(overlay_name);sequenceRenderer._peptide_sequences['atpeptide'].push(peptide);}}
...
if(this.result._peptide_sequences){sequenceRenderer._peptide_sequences['pep2pro']=this.result._peptide_sequences;}});return this;};MASCP.Pep2ProReader.prototype._normalise=function(array)
...
var peps=this.result.getPeptides();sequenceRenderer._peptide_sequences['ppdb']=[];var experiments=this.result.getExperiments();for(var i=0;i<experiments.length;i++){var layer_name='ppdb_experiment'+experiments[i];MASCP.registerLayer(layer_name,{'fullname':'Experiment '+experiments[i],'group':'ppdb','color':'#aa9900','css':css_block});MASCP.getLayer(layer_name).href='http://ppdb.tc.cornell.edu/dbsearch/searchsample.aspx?exprid='+experiments[i];for(var j=0;j<peps.length;j++){var peptide=peps[j];if(peps[j].experiments.indexOf(experiments[i])<0){continue;}
var peptide_bits=sequenceRenderer.getAminoAcidsByPeptide(peptide.sequence);peptide_bits.addToLayer(layer_name);peptide_bits.addToLayer(overlay_name);sequenceRenderer._peptide_sequences['ppdb'].push(peptide.sequence);}}
...
return gradient;};canvas.mod_gradient=function(id,stopObject,seqLength){var gradient=this.makeEl('linearGradient',{'id':id,'x1':'0%','x2':'100%','y1':'0%','y2':'0%'});for(var stop in stopObject){var thisPercent=stop/seqLength;var colorStr=''+255-(Math.round((stopObject[stop]/100)*255));gradient.appendChild(this.makeEl('stop',{'offset':thisPercent.toString(),'style':'stop-color:rgb(255,'+colorStr+','+colorStr+');stop-opacity:1'}));}
return gradient;};canvas.path=function(pathdesc){var a_path=document.createElementNS(svgns,'path');a_path.setAttribute('d',pathdesc);a_path.setAttribute('stroke','#000000');a_path.setAttribute('stroke-width','1');this.appendChild(a_path);return a_path;};canvas.poly=function(points){var a_poly=document.createElementNS(svgns,'polygon');a_poly.setAttribute('points',points);this.appendChild(a_poly);return a_poly;};canvas.circle=function(x,y,radius){var a_circle=document.createElementNS(svgns,'circle');a_circle.setAttribute('cx',typeof x=='string'?x:x*RS);a_circle.setAttribute('cy',typeof y=='string'?y:y*RS);a_circle.setAttribute('r',typeof radius=='string'?radius:radius*RS);this.appendChild(a_circle);return a_circle;};canvas.group=function(){var a_g=document.createElementNS(svgns,'g');this.appendChild(a_g);a_g.push=function(new_el){a_g.appendChild(new_el);};return a_g;};canvas.line=function(x,y,x2,y2){var a_line=document.createElementNS(svgns,'line');a_line.setAttribute('x1',typeof x=='string'?x:x*RS);a_line.setAttribute('y1',typeof y=='string'?y:y*RS);a_line.setAttribute('x2',typeof x2=='string'?x2:x2*RS);a_line.setAttribute('y2',typeof y2=='string'?y2:y2*RS);this.appendChild(a_line);return a_line;};canvas.rect=function(x,y,width,height){var a_rect=document.createElementNS(svgns,'rect');a_rect.setAttribute('x',typeof x=='string'?x:x*RS);a_rect.setAttribute('y',typeof y=='string'?y:y*RS);a_rect.setAttribute('width',typeof width=='string'?width:width*RS);a_rect.setAttribute('height',typeof height=='string'?height:height*RS);a_rect.setAttribute('stroke','#000000');this.appendChild(a_rect);return a_rect;};canvas.popup=function(mouseX,mouseY,clientX,popupData){var actualMouseX=mouseX-7;var actualMouseY=mouseY-130;if(clientX>=window.innerWidth/2){var offsetX=-370;var polyOffsetX=-20;}
else{var offsetX=20;var polyOffsetX=20;}
var a_popup=document.createElementNS(svgns,'g');a_popup.setAttribute('id','popup');var popup_rect=document.createElementNS(svgns,'rect');popup_rect.setAttribute('x',actualMouseX+offsetX);popup_rect.setAttribute('width',350);popup_rect.setAttribute('rx',10);popup_rect.setAttribute('ry',10);popup_rect.setAttribute('stroke','#000000');popup_rect.setAttribute('stroke-width',2);popup_rect.setAttribute('fill','#ffffff');a_popup.appendChild(popup_rect);var popup_poly=document.createElementNS(svgns,'polygon');var polyX=(actualMouseX+polyOffsetX);popup_poly.setAttribute('points',(actualMouseX)+','+(actualMouseY)+' '+polyX+','+(actualMouseY-10)+' '+polyX+','+(actualMouseY+10));popup_poly.setAttribute('stroke','#000000');popup_poly.setAttribute('stroke-width',2);popup_poly.setAttribute('fill','#ffffff');a_popup.appendChild(popup_poly);var popup_line=document.createElementNS(svgns,'polyline');popup_line.setAttribute('points',(polyX)+','+(actualMouseY-9)+' '+(polyX)+','+(actualMouseY+9));popup_line.setAttribute('stroke','#ffffff');popup_line.setAttribute('stroke-width',3);a_popup.appendChild(popup_line);var k=0;var m=0;if(popupData){var textX=actualMouseX+offsetX+10;lineHeight=12;lineLength=38;fontSize='14px';var popupTextContainer=document.createElementNS(svgns,'text');popupTextContainer.setAttribute('x',textX);popupTextContainer.setAttribute('font-family','monospace');popupTextContainer.setAttribute('font-size',fontSize);popupTextContainer.setAttribute('pointer-events','visiblePainted');a_popup.appendChild(popupTextContainer);var firstTitle=true;for(var popupKey in popupData){m++;var popupValue=new String(popupData[popupKey]);var newPopupElTitle=document.createElementNS(svgns,'tspan');newPopupElTitle.textContent=popupKey+': ';newPopupElTitle.setAttribute('x',textX);newPopupElTitle.setAttribute('dy',(firstTitle==true?0:lineHeight+4));popupTextContainer.appendChild(newPopupElTitle);var titleLength=popupKey.length;var titleWidth=(titleLength*8)+16;var contentLength=lineLength-titleLength;var firstLine=true;var contentWritten=0;var contentSize=popupValue.length;var lastIndex=0;while(lastIndex<contentSize){var newPopupElLine=document.createElementNS(svgns,'tspan');newPopupElLine.setAttribute('x',textX+titleWidth);newPopupElLine.setAttribute('dy',(firstLine==true?0:lineHeight));var thisLineLength=0;if(lastIndex+contentLength>=contentSize){thisLineLength=contentLength;}else{for(var l=contentLength;l>0;l--){var searchIndex=lastIndex+l;if(popupValue.substring(searchIndex,searchIndex+1)==' '){break;}}
thisLineLength=(l==0?contentLength:l);}
newPopupElLine.textContent=popupValue.substring(lastIndex,lastIndex+thisLineLength).trim();lastIndex+=thisLineLength;popupTextContainer.appendChild(newPopupElLine);if(firstLine==true){firstLine=false;}
k++;}
if(firstTitle==true){firstTitle=false;}}}
var popupHeight=(k*12)+((m>=1?m-1:0)*4)+30;popup_rect.setAttribute('height',popupHeight);popupTextContainer.setAttribute('y',actualMouseY-(popupHeight/2)+20);popup_rect.setAttribute('y',actualMouseY-(popupHeight/2));this.parentNode.appendChild(a_popup);return a_popup;};canvas.use=function(ref,x,y,width,height){var a_use=document.createElementNS(svgns,'use');a_use.setAttribute('x',typeof x=='string'?x:x*RS);a_use.setAttribute('y',typeof y=='string'?y:y*RS);a_use.setAttribute('width',typeof width=='string'?width:width*RS);a_use.setAttribute('height',typeof height=='string'?height:height*RS);a_use.setAttributeNS('http://www.w3.org/1999/xlink','href',ref);this.appendChild(a_use);return a_use;};canvas.a=function(href){var a_anchor=document.createElementNS(svgns,'a');a_anchor.setAttribute('target','_new');a_anchor.setAttributeNS('http://www.w3.org/1999/xlink','href',href);this.appendChild(a_anchor);return a_anchor;};canvas.button=function(x,y,width,height,text){var fo=document.createElementNS(svgns,'foreignObject');fo.setAttribute('x',0);fo.setAttribute('y',0);fo.setAttribute('width',x+width);fo.setAttribute('height',y+height);fo.style.position='absolute';this.appendChild(fo);var button=document.createElement('button');button.style.display='block';button.style.position='relative';button.style.top=y+'px';button.style.left=x+'px';button.textContent=text;fo.appendChild(button);return button;};canvas.svgbutton=function(x,y,width,height,txt){var button=this.group();var back=this.rect(x,y,width,height);back.setAttribute('rx','10');back.setAttribute('ry','10');back.setAttribute('stroke','#ffffff');back.setAttribute('stroke-width','2');back.setAttribute('fill','url(#simple_gradient)');x=back.x.baseVal.value;y=back.y.baseVal.value;width=back.width.baseVal.value;height=back.height.baseVal.value;var text=this.text(x+width/2,y+(height/3),txt);text.setAttribute('text-anchor','middle');text.firstChild.setAttribute('dy','1.5ex');text.setAttribute('font-size',0.5*height);text.setAttribute('fill','#ffffff');button.push(back);button.push(text);button.background_element=back;button.text_element=text;button.setAttribute('cursor','pointer');var button_trigger=function(){back.setAttribute('fill','#999999');back.setAttribute('stroke','#000000');};button.addEventListener('mousedown',button_trigger,false);button.addEventListener('touchstart',button_trigger,false);var button_reset=function(){back.setAttribute('stroke','#ffffff');back.setAttribute('fill','url(#simple_gradient)');};button.addEventListener('mouseup',button_reset,false);button.addEventListener('mouseout',button_reset,false);button.addEventListener('touchend',button_reset,false);return button;};canvas.callout=function(x,y,content,opts){var callout=this.group();var back=this.rect(-0.5*(opts.width+4),20,opts.width+4,opts.height+4);back.setAttribute('fill','#000000');var pres_box=this.rect(-0.5*(opts.width+1),22,opts.width+1,opts.height);pres_box.setAttribute('fill','#eeeeee');callout.push(back);callout.push(pres_box);var poly=this.poly('0,500 500,1000 -500,1000');poly.setAttribute('fill','#000000');callout.push(poly);var fo=document.createElementNS(svgns,'foreignObject');fo.setAttribute('x',-0.5*(opts.width+1)*RS);fo.setAttribute('y',21*RS);fo.setAttribute('width',opts.width*RS);fo.setAttribute('height',opts.height*RS);callout.push(fo);var html=document.createElementNS('http://www.w3.org/1999/xhtml','html');html.setAttribute('xmlns','http://www.w3.org/1999/xhtml');var body=document.createElementNS('http://www.w3.org/1999/xhtml','body');body.style.fontSize=(15*RS)+'px';body.style.margin=(5*RS)+'px';html.appendChild(body);body.appendChild(content);fo.appendChild(html);callout.setAttribute('transform','translate('+(x*RS)+','+((y+20)*RS)+')');return callout;};canvas.growingMarker=function(x,y,symbol,opts){var container=document.createElementNS(svgns,'svg');container.setAttribute('viewBox','-50 -100 200 250');container.setAttribute('preserveAspectRatio','xMinYMin meet');container.setAttribute('x',x);container.setAttribute('y',y);var the_marker=this.marker(50/RS,(50)/RS,50/RS,symbol,opts);container.appendChild(the_marker);container.contentElement=the_marker.contentElement;var result=this.group();var positioning_group=this.group();result.appendChild(positioning_group);positioning_group.appendChild(container);container.setAttribute('width','200');container.setAttribute('height','250');result.setAttribute('height','250');result.setAttribute('transform','scale(1)');result.setHeight=function(height){var scale_val=setHeight.call(this,height);this.setAttribute('height',height);var top_offset=this.offset;try{var widget_width=this.firstChild.firstChild.getBBox().width;}catch(e){console.log('getBBox() threw error in canvas.growingMarker');}
var widget_height=parseFloat(this.firstChild.firstChild.getAttribute('height'));var centering_offset=3/5*widget_height;if(this.angle>10){centering_offset=0;}
if(top_offset==0){centering_offset=0;}
if(this.setHeight!=arguments.callee){scale_val=1;}
this.firstChild.setAttribute('transform','translate(-100,'+(top_offset*RS/scale_val-centering_offset)+') rotate('+this.angle+',100,0)');};return result;};canvas.marker=function(cx,cy,r,symbol,opts){var units=0;if(typeof cx=='string'){var parts=new RegExp(/(\d+)(.*)/g).exec(cx);units=parts[2];cx=parseFloat(parts[1]);parts=new RegExp(/(\d+)(.*)/g).exec(cy);cy=parseFloat(parts[1]);parts=new RegExp(/(\d+)(.*)/g).exec(r);r=parseFloat(parts[1]);}
var dim={CX:cx+units,CY:cy+units,R:r+units,MIN_X:(cx-r)+units,MAX_X:(cx+r)+units,MIN_Y:(cy-r)+units,MAX_Y:(cy+r)+units,MID_X1:(cx-(r/2))+units,MID_X2:(cx+(r/2))+units,MID_Y1:(cy-(r/2))+units,MID_Y2:(cy+(r/2))+units};var marker=this.group();var fill_color=(opts&&opts.border)?opts.border:'rgb(0,0,0)';marker.push(this.circle(0,-0.5*r,r));marker.lastChild.style.fill=fill_color;marker.push(this.circle(0,1.5*r,r));marker.lastChild.style.fill=fill_color;var arrow=this.poly((-0.9*r*RS)+','+(0*r*RS)+' 0,'+(-2.5*r*RS)+' '+(0.9)*r*RS+','+(0*r*RS));arrow.setAttribute('style','fill:'+fill_color+';stroke-width: 0;');marker.push(arrow);marker.setAttribute('transform','translate('+((cx)*RS)+','+0.5*cy*RS+') scale(1)');marker.setHeight=setHeight;marker.setAttribute('height',dim.R*RS);if(typeof symbol=='string'){if(symbol=='CHO'){marker.contentElement=this.long_text_circle(0,0.5*r,1.75*r,symbol,opts);marker.push(marker.contentElement);}else{marker.contentElement=this.text_circle(0,0.5*r,1.75*r,symbol,opts);marker.push(marker.contentElement);}}else{marker.contentElement=this.group();if(symbol){marker.contentElement.push(symbol);}
...
results.sort(function(a,b){if(a[0]<b[0]){return-1;}else{return 1;}});return results;};var numPeps=0;for(var reader in this._peptide_sequences){var thisNumPeps=this._peptide_sequences[reader].length;var rdrCoverageList=[];numPeps+=thisNumPeps;if(thisNumPeps>0){this._peptide_sequences[reader]=convertToIndeces(this._peptide_sequences[reader]);for(var o=0;o<this._peptide_sequences[reader].length;o++){for(var p=this._peptide_sequences[reader][o][0];p<this._peptide_sequences[reader][o][1];p++){if(reader.indexOf('proteotypic')>=0){this._sequence_els[p]._predicted_coverage+=1;}else{this._sequence_els[p]._gator_coverage+=1;if(rdrCoverageList.indexOf(p)<0){this._sequence_els[p]._reader_coverage+=1;rdrCoverageList.push(p);}}}}}}
if(numPeps>0){var stopObject={};var maxRating=numPeps/40;maxRating=(maxRating>1)?1:maxRating;for(var q=0;q<this._sequence_els.length;q++){stopObject[q]=0;var thisRating=0;thisRating+=(4-this._sequence_els[q]._gator_coverage)*9;thisRating+=this._sequence_els[q]._predicted_coverage*22;thisRating-=this._sequence_els[q]._reader_coverage*20;thisRating=Math.max(Math.min(thisRating,100),0);thisRating=thisRating*maxRating;stopObject[q]=thisRating;}
...
if(layerName.charAt(0)=='g'){var tracer_marker=canvas.marker(this._index+0.3,10,0.5,'CHO');}else{var tracer_marker=canvas.marker(this._index+0.3,10,0.5,layerName.charAt(0).toUpperCase());}
tracer_marker.setAttribute('visibility','hidden');this._renderer._layer_containers[layerName].tracers.push(tracer);this._renderer._layer_containers[layerName].tracers.push(bobble);this._renderer._layer_containers[layerName].push(tracer_marker);canvas.tracers.push(tracer);return[tracer,tracer_marker,bobble];};var mouseOver=function(setting,target,canvas,popupData){if(setting=='on'){target.timerID=setTimeout(function(){canvas.cursorPos=canvas.cursorPos.matrixTransform(canvas.parentNode.getCTM().inverse());target.popup=canvas.popup(canvas.cursorPos.x,canvas.cursorPos.y,canvas.clientX,popupData);bean.add(target.popup,'mouseenter',function(){canvas.withinPopup=true;});bean.add(target.popup,'mouseleave',function(){canvas.withinPopup=false;setTimeout(function(){if('popup'in canvas.parentNode.childNodes&&!canvas.withinPopup===true){canvas.parentNode.removeChild(target.popup);}},100);});},500);}
if(setting=='off'){if(target.timerID){clearTimeout(target.timerID);}
setTimeout(function(){if('popup'in canvas.parentNode.childNodes&&!canvas.withinPopup===true){canvas.parentNode.removeChild(target.popup);}},20);}
return;};var addBoxOverlayToElement=function(layerName,width,fraction){if(typeof fraction=='undefined'){fraction=1;}
...
var peptideSequence='';var popupData={};var rect=canvas.rect(-0.25+this._index,60,width||1,4);rect.position_start=this._index;rect.position_end=this._index+width;var rect_x=parseFloat(rect.getAttribute('x'));var rect_max_x=rect_x+parseFloat(rect.getAttribute('width'));var container=this._renderer._layer_containers[layerName];var exitLoop=false;for(var i=0;i<container.length;i++){var el_x=parseFloat(container[i].getAttribute('x'));var el_max_x=el_x+parseFloat(container[i].getAttribute('width'));if((el_x<=rect_x&&rect_x<=el_max_x)||(rect_x<=el_x&&el_x<=rect_max_x)){if(container[i].style.opacity!=fraction){continue;}
var newX=Math.min(el_x,rect_x);var newWidth=Math.max(el_max_x,rect_max_x)-Math.min(el_x,rect_x);container[i].setAttribute('x',""+newX);container[i].setAttribute('width',""+newWidth);var newStart=Math.min(container[i].position_start,this._index);var newEnd=Math.max(container[i].position_end,this._index+width);container[i].position_start=newStart;container[i].position_end=newEnd;peptideSequence=this._renderer.sequence.slice(newStart,newEnd);popupData['Sequence']=peptideSequence;bean.remove(container[i],'mouseenter');bean.add(container[i],'mouseenter',function(){mouseOver('on',container[i],canvas,popupData);});rect.parentNode.removeChild(rect);var noOverlap=false;var persistentBoxOverlay=container[i];do{for(var j=0;j<container.length;j++){var el_x_2=parseFloat(container[j].getAttribute('x'));var el_max_x_2=el_x_2+parseFloat(container[j].getAttribute('width'));if(((el_x_2<=newX&&newX<=el_max_x_2)||(newX<=el_x_2&&el_x_2<=(newX+newWidth)))&&!(el_x_2==newX&&el_max_x_2==newX+newWidth)){newX=Math.min(el_x_2,newX);newWidth=Math.max(el_max_x_2,(newX+newWidth))-Math.min(el_x_2,newX);persistentBoxOverlay.setAttribute('x',""+newX);persistentBoxOverlay.setAttribute('width',""+newWidth);newStart=Math.min(persistentBoxOverlay.position_start,container[j].position_start);newEnd=Math.max(persistentBoxOverlay.position_end,container[j].position_end);persistentBoxOverlay.position_start=newStart;persistentBoxOverlay.position_end=newEnd;peptideSequence=this._renderer.sequence.slice(newStart,newEnd);popupData['Sequence']=peptideSequence;bean.remove(persistentBoxOverlay,'mouseenter');bean.add(persistentBoxOverlay,'mouseenter',function(){mouseOver('on',persistentBoxOverlay,canvas,popupData);});container.splice(j,1);j=0;}
else if(j+1==container.length){noOverlap=true;}}}while(noOverlap==false);return persistentBoxOverlay;}}
this._renderer._layer_containers[layerName].push(rect);rect.setAttribute('class',layerName);rect.style.strokeWidth='0px';rect.setAttribute('visibility','hidden');rect.style.opacity=fraction;rect.setAttribute('fill',MASCP.layers[layerName].color);rect.setAttribute('cursor','default');peptideSequence=this._renderer.sequence.slice(this._index,this._index+width);popupData['Sequence']=peptideSequence;bean.add(rect,'mouseenter',function(){mouseOver('on',rect,canvas,popupData);});bean.add(rect,'mouseleave',function(){mouseOver('off',rect,canvas,popupData);});return rect;};var addElementToLayerWithLink=function(layerName,url,width){var canvas=this._renderer._canvas;if(!canvas){var orig_func=arguments.callee;var self=this;this._renderer.bind('sequencechange',function(){this._renderer.unbind('sequencechange',arguments.callee);orig_func.call(self,layerName,url,width);});log("Delaying rendering, waiting for sequence change");return;}
