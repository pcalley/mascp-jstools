...
positions=positions.sort();for(var i=positions.length-1;i>=0;i--){var j=i-1;pos=positions[i];var value=inserts[pos];while(j>=0){pos-=inserts[positions[j]].length;j--;}
result_data[pos]=value;}
delete result_data[-1];return result_data;};var splice_char=function(seqs,index,insertions){for(var i=0;i<seqs.length;i++){var seq=seqs[i];if(seq.charAt(index)!='-'){if(!insertions[i]){insertions[i]={};insertions[i][-1]='';}
