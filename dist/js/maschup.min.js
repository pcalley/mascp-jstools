...
if((!this._SERVER_DATASETS)&&agi&&agi!="dummy"){var read=new MASCP.ArbitraryDataReader("",self._endpointURL);read.retrieve("dummy",function(){self._SERVER_DATASETS=this.result._raw_data.data;self.retrieve(in_agi,cback);});return;}
...
if(this._SERVER_DATASETS.length==0){MASCP.Service.prototype.retrieve.call(self,"dummy",cback);return;}
this._SERVER_DATASETS.forEach(function(set){var reader=self._extend(set);(self.renderers||[]).forEach(function(rrend){reader.setupSequenceRenderer(rrend);rrend.bind('resultsRendered',function(e,rdr){if(rdr==reader){jQuery(rrend).trigger('resultsRendered',[self]);}});});reader.retrieve(agi,cback);});};MASCP.ArbitraryDataReader.Result=MASCP.ArbitraryDataReader.Result;MASCP.ArbitraryDataReader.Result.prototype.getPeptides=function()
